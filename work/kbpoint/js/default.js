...
function viewProductPopup() { //제품상세 팝업
  window.open('viewProductPopup.php','viewProductPopup','scrollbars=no,width=765,height=715,left=100,top=100');
}

function preCalcPopup() { //미리계산 팝업
  window.open('preCalc.php','preCalcPopup','scrollbars=no,width=615,height=530,left=300,top=300');
}

function calendarPop() {//달력 팝업
  window.open('calendarPop.php','calendarPop','scrollbars=no,width=198,height=175,left=300,top=300');
}

function zipcodePop(){ //우편번호찾기 팝업 (가운데 띄우기)

   window.open('zipcodePop.php','zipcodePop','scrollbars=no,width=405,height=383,left=300,top=300');
}
...
function setPng24(obj) { // png 파일 패치
    ...
    obj.src='http://image.hmall.com/static/b2b/image/KP/images/blank.gif';  
    return ''; 
}
...
window.onscroll = function(){
    if(document.getElementById("divQuickTop") != null){
        ...
        if (scrollTop == 0)
        {
            QuicknewPos = 150 + document.body.scrollTop;
        }
        else{
            QuicknewPos = scrollTop + 150;
        }
