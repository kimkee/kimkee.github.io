...
%>
					...
					<div class="ctnBox itemMall">
						...
						<div class="itemTab">
							<ul>
								<li class="on"><a href="/work/lunaPlus/itemMall/cashList1.php">충전내역</a></li>
								<li><a href="/work/lunaPlus/itemMall/cashList2.php">사용내역</a></li>
							</ul>
						</div>
...
url = "http://218.234.20.212:8000/ansi_api/API_BP_PAYHIST_SEARCH_VER3.php?cnt_for_view="&pgsize&"&no_page="&page&"&no_user="&Enpang_Idx&""
url = url & "&id_user="&Login_ID&"&cd_company=EYAH&yyyymmdd_s="&yyyymmdd_s&"&yyyymmdd_e="&yyyymmdd_e&"&status=TTTT"
...
If myRec.selectSingleNode("//root/Return_Code").Text = "0000" Then
	...
	TOT_COUNT = myRec.selectSingleNode("//root/Data/TOT_COUNT ").Text
	 ...
	 For i=0 To Nodes.length-1
		NM_CASH = Nodes(i).selectSingleNode("NM_CASH").Text
		...
		DT_END = Nodes(i).selectSingleNode("DT_END").Text
		DT_DATE = Nodes(i).selectSingleNode("DT_DATE").Text
		TM_TIME = Nodes(i).selectSingleNode("TM_TIME").Text

		DT_END_Year = left(DT_END,4)
		DT_END_Monteh = Mid(DT_END,5,2)
		DT_END_Day = "." & Right(DT_END,2)
		DT_END = DT_END_Year &"."& DT_END_Monteh & DT_END_Day

		DT_DATE_Year = Mid(DT_DATE,3,2)
